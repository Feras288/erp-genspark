// =====================================================
// Phase 11B-B-3: purchase_invoice_received auto-post.
//
// Inserts exactly one POSTED JournalEntry per
// (companyId, PURCHASE_INVOICE, invoice.id) inside the
...
export type PostPurchaseInvoiceReceivedResult = {
  id: string;
  reused: boolean;
};
...
export async function postPurchaseInvoiceReceived(
  tx: Prisma.TransactionClient,
  args: {
    companyId: string;
    userId: string;
    invoice: PurchaseInvoiceReceivedSource;
  },
): Promise<PostPurchaseInvoiceReceivedResult> {
  ...
  const template = buildPurchaseInvoiceReceivedTemplate(args.invoice);
  const lines = omitZeroAmountLines(template.lines);
  if (lines.length < 2) {
    throw new BadRequestException(
      'Purchase invoice posting requires at least 2 non-zero journal lines',
    );
  }
