// =====================================================
// Phase 11B-B-4: ar_payment_posted auto-post.
//
// Inserts exactly one POSTED JournalEntry per
// (companyId, AR_PAYMENT, payment.id) inside the
...
export type PostArPaymentPostedResult = {
  id: string;
  reused: boolean;
};
...
export async function postArPaymentPosted(
  tx: Prisma.TransactionClient,
  args: {
    companyId: string;
    userId: string;
    payment: ArPaymentPostedSource;
  },
): Promise<PostArPaymentPostedResult> {
  ...
  const template = buildArPaymentPostedTemplate(args.payment);
  const lines = omitZeroAmountLines(template.lines);
  if (lines.length < 2) {
    throw new BadRequestException(
      'AR payment posting requires at least 2 non-zero journal lines',
    );
  }
